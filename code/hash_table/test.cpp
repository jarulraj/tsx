...
{
    HashTable ht;
    ht.ht_init(&ht, (ht_flags)(HT_KEY_CONST | HT_VALUE_CONST), 0.05);

    char *s1 = (char*)"teststring 1";
    char *s2 = (char*)"teststring 2";
    char *s3 = (char*)"teststring 3";

    ht.ht_insert(&ht, s1, strlen(s1)+1, s2, strlen(s2)+1);

    int contains = ht.ht_contains(&ht, s1, strlen(s1)+1);
    test(contains, "Checking for key \"%s\"", s1);

    size_t value_size;
    char *got = (char*) ht.ht_get(&ht, s1, strlen(s1)+1, &value_size);
    ...
    ht.ht_insert(&ht, s1, strlen(s1)+1, s3, strlen(s3)+1);

    unsigned int num_keys;
    void **keys;

    keys = ht.ht_keys(&ht, &num_keys);
    test(num_keys == 1, "HashTable has %d keys", num_keys);

    test(keys != NULL, "Keys is not null");
    if(keys)
      free(keys);
    got = (char*) ht.ht_get(&ht, s1, strlen(s1)+1, &value_size);
    ...
    ht.ht_remove(&ht, s1, strlen(s1)+1);

    contains = ht.ht_contains(&ht, s1, strlen(s1)+1);
    ...
    for(i = 0; i < key_count; i++)
    {
        ht.ht_insert(&ht, &(many_keys[i]), sizeof(many_keys[i]), &(many_values[i]), sizeof(many_values[i]));
    }
    ...
    for(i = 0; i < key_count; i++)
    {
        if(ht.ht_contains(&ht, &(many_keys[i]), sizeof(many_keys[i])))
        {
            size_t value_size;
            int value;

            value = *(int*)ht.ht_get(&ht, &(many_keys[i]), sizeof(many_keys[i]), &value_size);
    ...
    for(i = 0; i < key_count; i++)
    {
        ht.ht_remove(&ht, &(many_keys[i]), sizeof(many_keys[i]));
    }
    test(ht.ht_size(&ht) == 0, "%d keys remaining", ht.ht_size(&ht));
    ht.ht_destroy(&ht);
    free(many_keys);
    free(many_values);
